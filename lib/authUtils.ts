...
export async function getUserFromRequest(request: NextRequest) {
  try {
    ...
    if (error) {
      console.warn(
        "Warning: Could not get user from request using standard method:",
        error?.message || error,
      );
    ...
    console.warn(
      "Warning: Exception getting user from request:",
      error?.message || error,
    );
    // Log the available cookies for debugging
    // console.log('Available cookies in request:', request.cookies.getAll().map(c => c.name));
    ...
    if (supabaseUrl) {
      ...
      if (authCookie) {
        try {
          ...
          if (authData?.user) {
            console.log(
              "Successfully extracted user from auth cookie as fallback",
            );
            return authData.user;
          }
        } catch (parseError) {
          console.error(
            "Error parsing auth cookie in catch block:",
            parseError,
          );
        }
      }
